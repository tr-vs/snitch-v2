...
class JuulStealCommand extends Command {
    ...
    async exec(message) {
        let [hits, steals, menthol, mango, cucumber, creme, fruit] = await econ.getBal(message.author.id, message.guild.id)
        if (steals == 0) {
        ...
        if (message.guild.members.cache.get(check.juulHolder) == undefined) {
            ...
            let steals = await econ.useSteal(message.author.id, message.guild.id)
